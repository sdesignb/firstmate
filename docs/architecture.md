...
Crewmate and scout dispatch can use the static adapter resolved by `config/crew-harness` plus an explicitly chosen model, or it can use local dispatch profiles in `config/crew-dispatch.json`.
The dispatch file is intentionally judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current quota output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, and passes a concrete harness and model plus any chosen effort and model provenance to `fm-spawn.sh`.
The shell scripts validate the JSON shape, concrete permitted models, and verified harness/effort combinations, but they do not parse task intent, match natural-language rules, or own array selection.
...
Secondmate launches are exempt from profile matching because they resolve the secondmate harness and require a concrete model from its optional config token or the spawn request instead.
...
A bare harness line remains harness-only and requires an explicit `--model` at spawn.
