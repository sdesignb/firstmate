...
1. **Resolve the profile.**
   ...
   A harness change requires a new model and resets effort unless those axes are named too, because a model chosen for one adapter does not transfer to another and an implicit model is refused.
2. **Safe checkpoint.**
