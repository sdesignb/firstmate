...
A bare `<harness>` (for example `claude`) resolves only the adapter; the spawn must also receive an explicit `--model`, because implicit harness model defaults are prohibited.
