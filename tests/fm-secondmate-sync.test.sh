...
test_nudge_retry_uses_fresh_herdr_endpoint_after_respawn() {
  ...
  meta="$w/home/state/sm-instr.meta"
  {
    ...
    printf 'model_source=task-metadata\n'
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" sm "$w/sm" codex --secondmate --model test-model >/dev/null 2>&1 || true
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    FM_PROJECTS_OVERRIDE="$w/home/projects" FM_CONFIG_OVERRIDE="$w/home/config" \
    FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" sm "$w/sm" codex --secondmate --model test-model >/dev/null 2>"$err" || true
