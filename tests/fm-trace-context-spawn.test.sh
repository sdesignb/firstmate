...
run_spawn() {
  ...
  env -u FM_TRACE_CONTEXT \
    ...
    "$SPAWN" "$@" --model test-model --mode no-mistakes --yolo off 2>&1
}
...
run_two_level() {
  ...
  env FM_TRACE_CONTEXT="$penv" \
    ...
    "$SPAWN" "$sm_id" "$sm" --secondmate --model test-model >/dev/null 2>&1 || true
  ...
  env FM_TRACE_CONTEXT="$TL_ENV_TC" TRACEPARENT="$TL_CARRIER" \
    ...
    "$SPAWN" "$worker_id" "$wproj" --model test-model --mode no-mistakes --yolo off >/dev/null 2>&1 || true
...
test_duplicate_secondmate_spawn_does_not_converge_trace_context() {
  ...
  out=$(env -u FM_TRACE_CONTEXT \
    ...
    PATH="$fake:$PATH" "$SPAWN" "$id" "$sm" --secondmate --model test-model 2>&1)
  status=$?
