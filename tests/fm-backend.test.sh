...
run_spawn_symlink_case() {  # <label> <physical|logical>
  ...
  out=$(run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude --model test-model --mode no-mistakes --yolo off 2>&1)
  rc=$?
...
test_spawn_default_backend_writes_no_meta_field() {
  ...
  out=$(PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" claude --model test-model --mode no-mistakes --yolo off --backend tmux 2>&1)
...
test_spawn_autodetect_nesting_resolves_tmux_silently() {
  ...
  out=$(PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" claude --model test-model --mode no-mistakes --yolo off 2>&1)
