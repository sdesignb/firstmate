...
spawn_from_launcher() {
  ...
  if [ -n "$pane" ]; then
    env HERDR_ENV=1 HERDR_PANE_ID="$pane" HERDR_SESSION="$HERDR_LAB_SESSION" \
      ...
      "$ROOT/bin/fm-spawn.sh" "$id" "$proj" "echo launcher-ws-ok --model __MODEL__" --model test-model --model-source raw-launch-command --backend herdr "$@" \
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/fm-spawn.sh" dupC "$PROJ" "echo launcher-ws-ok --model __MODEL__" --model test-model --model-source raw-launch-command --mode no-mistakes --yolo off --backend herdr \\
